...
pressedStart = False       # Start button flag
lastAngle = 0              # Last servo angle (used to avoid small jitter)
...
# I2C multiplexer and MPU
MUX_ADDR = 0x70
MUX_CHANNEL_3 = 0x08
I2C_BUS = 1
MPU_ADDR = 0x68

# XSHUT pins for sensor reset
XSHUT_PINS = [8, 7, 1, 25]
...
# Sensor index → mux channel mapping
order = [0, 1, 4, 2]
sensors = [None] * 4
...
def SetAngle(angle):
    # Set steering angle (-45 to 45) with smoothing
...
def init_sensor(index, channel):
    # Initialize sensor on given mux channel
    global sensors
    ...
    for attempt in range(5):
        try:
            ...
            print(f"Sensor {index} initialized")
            return True
...
def safe_read(sensor, timeout=0.10):
    # Read sensor in separate thread to prevent blocking
    result = [None]
...
def read_sensor(index):
    # Safe sensor read with auto-recovery
...
def unpark():
    # Initial movement to exit starting position
    timeOfMoveLeft = 0.40
    timeOfMoveRight = 0.55
    speed = 40
    ...
    if direction == 0:
        curDir = 0
        timeOfMove = timeOfMoveLeft
    else:
        curDir = 1
        timeOfMove = timeOfMoveRight
...
def main():
    ...
    timeAfterLastLine = 0.5


    # States:
    # 0 = wall following out
    # 1 = turning
    # 2 = approach semaphore
    # 3 = go around semaphore
    STATE = 0
    stateChange = time.perf_counter()
    ...
    semTurnTimeGreen1 = 1.30
    semTurnTimeGreen2 = 0.90
    semTurnTimeRed1 = 1.10
    semTurnTimeRed2 = 1.10

    semTurnColor = -1

    jiggleTime = 1.4
    lastJiggleTime = time.perf_counter()

    # Start with unparking and find direction
    # 0 - counterclockwise
    # 1 - clockwise
    direction = unpark()
    
    if direction == 1:
        timeAfterLastLine = 0.8
        distFromWall = 350
        turnTime = 1.2
    ...
    while True:
        ...
        # Detect line
        area = getArea(picam2,1 - direction)
        # Get semaphore data
        ...
        if area > 0:
            ...
            if corner and curTime - lastLine > timeAfterLastLine:
                if STATE == 0:
                    STATE = 1
                    stateChange = curTime
                corner = False
        ...
        if STATE == 3:
            ...
            if curTime - stateChange > totalTurnTime:
                ...
                if semTurnColor == 0:
                    if curTime - stateChange < semTurnTimeGreen1:
                        SetAngle(-45)
                        continue
                    else:
                        if semColor != -1:
                    ...
                    if curTime - stateChange < semTurnTimeRed1:
                        SetAngle(45)
                        continue
                    else:
                        if semColor != -1:
        ...
        #print(f"L:{distanceLeft} F:{distanceFront} R:{distanceRight}")

        # Skip loop if sensors invalid
        ...
        # -------- ACTUATION --------
        if STATE == 0:
            if distanceFront < distFromFront:
                if direction == 0:
                    stop()
                    time.sleep(1.0)
                    reverse(speed)
                    ...
                    SetAngle(45)
                    time.sleep(0.3)
            elif distanceRight > 2000 and direction == 0:
                if distanceLeft < 250:
                    SetAngle(20)
                else:
                    SetAngle(0)
            elif distanceLeft > 2000 and direction == 1:
                if distanceRight < 250:
                    SetAngle(-20)
                else:
                    SetAngle(0)
            else:
                if curTime - lastJiggleTime > jiggleTime:
                    if direction == 0:
                        SetAngle(-45)
                    else:
                        SetAngle(45)
                    lastJiggleTime = curTime
                else:
                    SetAngle(angle)
